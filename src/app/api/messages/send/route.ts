...
import { encryptAndSignMessage } from "@/lib/crypto/messaging";

export async function POST(req: NextRequest) {
  const { recipientUsername, message, password } = await req.json();
  ...
  const { data: recipientProfile } = await supabase
    .from("profiles")
    .select("username, public_key_armored")
  ...
  const encryptedPayload = await encryptAndSignMessage(message, recipientProfile.public_key_armored, senderKey);

  const { error: insertError } = await supabase.from("messages").insert({
    sender_id: user.id,
    recipient_id: (await supabase.from("profiles").select("id").eq("username", recipientUsername).single()).data?.id,
